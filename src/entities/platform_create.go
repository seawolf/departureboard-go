...
func (pc *PlatformCreate) SetNillableName(s *string) *PlatformCreate {
	if s != nil {
		pc.SetName(*s)
	}
	return pc
...
func (pc *PlatformCreate) check() error {
	if _, ok := pc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`entities: missing required field "Platform.name"`)}
	}
	return nil
}
